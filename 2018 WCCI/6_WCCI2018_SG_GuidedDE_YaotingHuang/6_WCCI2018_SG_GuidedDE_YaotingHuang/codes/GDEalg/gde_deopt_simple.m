...
function [Fit_and_p,FVr_bestmemit, fitMaxVector, Best_otherInfo] = ...
    gde_deopt_simple(gdeParameters,caseStudyData,otherParameters,low_habitat_limit,up_habitat_limit)

%-----This is just for notational convenience and to keep the code uncluttered.--------
I_NP         = gdeParameters.I_NP;
F_weight     = gdeParameters.F_weight;
F_CR         = gdeParameters.F_CR;
I_D          = numel(up_habitat_limit); %Number of variables or dimension
gdeParameters.nVariables=I_D;
FVr_minbound = low_habitat_limit;
FVr_maxbound = up_habitat_limit;
I_itermax    = gdeParameters.I_itermax;

%Repair boundary method employed
BRM=gdeParameters.I_bnd_constr; %1: bring the value to bound violated
                               %2: repair in the allowed range

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
I_strategy   = gdeParameters.I_strategy; %important variable
...
gdeParameters.minPositionsMatrix=minPositionsMatrix;
gdeParameters.maxPositionsMatrix=maxPositionsMatrix;
...
while gen<=I_itermax %%&&  fitIterationGap >= threshold
    ...
    %% Guide
    FM_ui = guide_trial(FM_ui, gdeParameters.R_rate, gdeParameters.R_dir);
    ...
    %%update_guide_direction
    gdeParameters.R_dir = update_guid_dir(FM_ui, solFitness_M_temp, gdeParameters.U_rate, gdeParameters.U_div, gdeParameters.R_dir);
...
function FM_ui=guide_trial(FM_ui, guide_rate, guide_dir)
    scale = sqrt(sum(FM_ui .^2, 2));
    vector = FM_ui ./ scale;
    new_vector = (1-guide_rate) * vector + guide_rate * guide_dir;
    FM_ui = new_vector .* scale;
return

function guide_dir=update_guid_dir(solutions, fitness, update_rate, par_div, old_guide_dir)
    scale = sqrt(sum(solutions .^ 2, 2));
    vector = solutions ./ scale;
    weight = mean(fitness, 2);
    weight = weight ./ scale;
    weight = exp(-weight / par_div);
    new_vector = vector .* weight;
    new_vector = sum(new_vector, 1);
    new_vector = new_vector / norm(vector);
    
    guide_dir = (1 - update_rate) * old_guide_dir + update_rate * new_vector;

...
pos(1,:)=xbest;
%pos(2,:)=xbest;
% for k = 2 : set_par.pop_size
% for i=L:U
%    for j=1:24
%       posj(k,i+((j-1)*142))=pos(k,j+(i-L)*24);
%    end
% end
% end
...
% for i=L:U
%    for j=1:24
%       posj(1,i+((j-1)*142))=xbestj(1,j+(i-L)*24);
%    end
% end
...
% (2*(deParameters.I_NP*deParameters.Scenarios))
...
%         for k = 1 : set_par.pop_size
%         for i=L:U
%         for j=1:24
%                 copyPosj(k,i+((j-1)*142))=copyPos(k,j+(i-L)*24);
%                 posj(k,i+((j-1)*142))=pos(k,j+(i-L)*24);
%         end
%         end
%         end
...
fitMaxVector(:,gen)=[gbestval;mean(solPenalties_M(I_best_index,:))]; %We save the mean value and mean penalty value
% The user can decide to save the mean, best, or any other value here
%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
Fit_and_p=[fitMaxVector(1,gen) p1]; %;p2;p3;p4]

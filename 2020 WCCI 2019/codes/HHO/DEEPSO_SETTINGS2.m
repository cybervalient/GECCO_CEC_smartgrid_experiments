...
function [Deepso,Ff,Set] = DEEPSO_SETTINGS2(low_habitat_limit,up_habitat_limit,deParameters,gbest)
...
Set.Xmax(round(Set.D*y):end)         = up_habitat_limit(round(Set.D*y):end);    % Individuals' lower bounds.
Set.Xmin(round(Set.D*y):end)         = low_habitat_limit(round(Set.D*y):end);    % Individuals' upper bounds.

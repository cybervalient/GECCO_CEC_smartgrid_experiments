...
[solFitness_M, solPenalties_M,Struct_Eval]=feval(fnc,FM_pop,caseStudyData, otherParameters,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while gen<=I_itermax %%&&  fitIterationGap >= threshold
    ...
    Neval = 10;
    if(gen ~= I_itermax)
        [solFitness_M_temp, solPenalties_M_temp,Struct_Eval_temp]=feval(fnc,FM_ui,caseStudyData, otherParameters,Neval);
    else
        [solFitness_M_temp, solPenalties_M_temp,Struct_Eval_temp]=feval(fnc,FM_ui,caseStudyData, otherParameters,100);
    end
    ...
    if(gen ~= I_itermax)
        ...
        for i=1:size(solFitness_M_temp,1)
            S_val_temp(i)=getObjectiveValue(solFitness_M_temp(i,:),Neval,2);
        end
    else
        [S_val_temp, worstS]=max(solFitness_M_temp,[],2); %Choose the solution with worse performance
        for i=1:size(solFitness_M_temp,1)
            S_val_temp(i)=getObjectiveValue(solFitness_M_temp(i,:),Neval,1);
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

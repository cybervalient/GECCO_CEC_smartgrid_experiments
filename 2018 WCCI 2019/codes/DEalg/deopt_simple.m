...
[solFitness_M, solPenalties_M,Struct_Eval]=feval(fnc,FM_pop,caseStudyData, otherParameters,4);
...
[S_val1, worstS]=min(solFitness_M,[],2); %Choose the solution with worse performance

S_val2 = mean(solFitness_M,2);
S_val3 = max(solFitness_M,[],2);
S_val4 = median(solFitness_M,2);
[~,I_best_index] = min(S_val3); % This mean that the best individual correspond to the best worst performance
...
while gen<=I_itermax %%&&  fitIterationGap >= threshold
    ...
    [solFitness_M_temp, solPenalties_M_temp,Struct_Eval_temp]=feval(fnc,FM_ui,caseStudyData, otherParameters,4);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% The user should decide which is the criterion to optimize. 
    % In this example, we optimize worse performance
    [S_val_temp1, worstS]=min(solFitness_M_temp,[],2); %Choose the solution with worse performance
    S_val_temp2 = mean(solFitness_M_temp,2);
    S_val_temp3 = max(solFitness_M_temp,[],2);
    S_val_temp4 = median(solFitness_M_temp,2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for jj = 1 : I_NP
    ft = rand;
    if ft <= 0.25
      if S_val_temp1(jj) <= S_val1(jj)
          FM_pop(jj,:) = FM_ui(jj,:);
          S_val2(jj) = S_val_temp2(jj);
          S_val3(jj) = S_val_temp3(jj);
          S_val4(jj) = S_val_temp4(jj);
          ind = [ind;jj];
      end
    elseif ft > 0.25 && ft <= 0.5
       if S_val_temp2(jj) <= S_val2(jj)
          FM_pop(jj,:) = FM_ui(jj,:);
          S_val1(jj) = S_val_temp1(jj);
          S_val3(jj) = S_val_temp3(jj);
          S_val4(jj) = S_val_temp4(jj);
          ind = [ind;jj];
      end 
    elseif ft > 0.5 && ft <= 0.75
        if S_val_temp3(jj) <= S_val3(jj)
          FM_pop(jj,:) = FM_ui(jj,:);
          S_val1(jj) = S_val_temp1(jj);
          S_val2(jj) = S_val_temp2(jj);
          S_val4(jj) = S_val_temp4(jj);
    ...
    elseif ft > 0.75 && ft <= 1.0
        if S_val_temp4(jj) <= S_val4(jj)
          FM_pop(jj,:) = FM_ui(jj,:);
          S_val2(jj) = S_val_temp2(jj);
          S_val3(jj) = S_val_temp3(jj);
          S_val1(jj) = S_val_temp1(jj);
    ...
    %% update best results
    [S_bestval,I_best_index] = min(S_val3);
    FVr_bestmemit = FM_pop(I_best_index,:); % best member of current iteration
...
function [FM_ui,FM_base,msg]=generate_trial(method,F_weight, F_CR, FM_pop, FVr_bestmemit,I_NP,I_D,FVr_rot)
  ...
  FM_mpo = FM_mui < 0.5;    % inverse mask to FM_mui
    ...
    switch method
        case 1,
            FM_ui = FM_pm3 + F_weight*(FM_pm1 - FM_pm2);   % differential variatio
